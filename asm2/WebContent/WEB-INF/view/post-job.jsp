				...
				<ul class="navbar-nav ml-auto">
					...
					<c:if test="${empty sessionScope.full_name}">
						<li class="nav-item cta cta-colored"><a
							href="/asm2/auth/login" class="nav-link">Đăng nhập</a></li>
					</c:if>

					<c:if test="${not empty sessionScope.full_name}">
						<li class="nav-item cta mr-md-1"><a class="nav-link"
							href="/asm2/user/profile">Hồ Sơ</a></li>
						<c:if test="${sessionScope.role eq  0}">
							<li class="nav-item active"><a
								href="/asm2/user/list-apply-job" class="nav-link">Công việc
									đã ứng tuyển</a></li>
						</c:if>
						...
						<c:if test="${sessionScope.role eq  1}">
							...
							<li class="nav-item cta mr-md-1"><a
								href="/asm2/recruitment/list-post" class="nav-link">Danh
									sách bài đăng</a></li>
						</c:if>

						<li class="nav-item active"><a href="/asm2/auth/logout"
...
<div class="hero-wrap hero-wrap-2" style="background-image: url('user/assets/images/bg_1.jpg');" data-stellar-background-ratio="0.5" th:if="${session.user.role.id == 1 }">
	...
	<section class="site-section" th:if="${session.user.role.id == 2 }">
		<div class="container">
			<form:form action="/asm2/recruitment/addRecruit" method="post"
						...
						<div class="p-4 p-md-5 border rounded" method="post">
							...
							<div class="form-group">
								...
								<form:input type="text" class="form-control" path="title"
									placeholder="Tiêu đề"></form:input>
							</div>
							<div class="form-group">
								<label>Mô tả công việc</label>
								<form:textarea path="description" class="form-control"
									id="editorN" placeholder="Mô tả"></form:textarea>
							</div>
							<div class="form-group">
								<label">Kinh nghiệm</label>
								<form:input type="text" class="form-control" id="job-title"
									path="experience" placeholder="Kinh nghiệm"></form:input>
							</div>
							<div class="form-group">
								<label>Số người cần tuyển</label>
								<form:input type="number" class="form-control" id="job-title"
									path="quantity" placeholder="Số người cần tuyển"></form:input>
							</div>
							<div class="form-group">
								<label>Địa chỉ</label>
								<form:input type="text" class="form-control" id="job-location"
									path="address" placeholder="Địa chỉ công ty"></form:input>
							</div>
							<div class="form-group">
								<label>Hạn ứng tuyển</label>
								<form:input type="date" class="form-control" id="job-location"
									path="deadline" placeholder="Địa chỉ công ty"></form:input>
							</div>
							<div class="form-group">
								<label for="job-location">Lương</label>
								<form:input type="text" class="form-control" id="job-location"
									path="salary" placeholder="Mức lương"></form:input>
							</div>
